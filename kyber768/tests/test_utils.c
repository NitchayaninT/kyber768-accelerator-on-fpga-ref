...
void get_test_result_path(char *test_result_path, char *filename) {
  const char *home = getenv("HOME");
  if (home) {
    snprintf(test_result_path, 256, "%s/kyber/data/test_result/%s", home,
             filename);
    printf("Test result from: %s\n", test_result_path);
  }
}
